...
class Admin {
  ...
  function __construct() {
   ...
   // load some html parts we are going to use
    require( 'admin/assets/php/content.php' );
  ...
  function showPage() {
	...
	if( !$login ){
	  	$output = $common->readHTML( 'admin/assets/html/login.html' );
    } elseif( isset($_REQUEST['action']) && $_REQUEST['action'] != '' && $_REQUEST['action'] != 'login') {
      if($_REQUEST['action'] == 'edit') {
   	    $output = $common->readHTML( 'admin/assets/html/' . $_REQUEST['page'] .'_'. $_REQUEST['action'] . '.html' );
		...
		exit();
   	  ...
   	  $output = $common->readHTML( 'admin/assets/html/' . $_REQUEST['page'] . '.html' );
	} else {
      $output = $common->readHTML( 'admin/assets/html/index.html' );
	}
  ...
  function updateOther( $output ){
    ...
    if( $_REQUEST['page'] == 'home' ) {
	  ...
	  $changelog = $common->getXML( 'http://www.templateblocks.com/support/wp-rss2.php?cat=3' );
	  
	  if( !empty($changelog) ) {
	    $Latest_version = str_replace( 'Version ', '', $changelog['ITEM']['TITLE'] );
      } else {
	    $Latest_version = 'N/A';
	  }
	  //Future versions will have more information dug from the main site
	  //$repository = $common->getXML( 'http://www.templateblocks.com/support/wp-rss2.php?cat=6' );
 	  //$comments = $common->getXML( 'http://www.templateblocks.com/support/comments/feed/' );
  ...
  function updateEdit( $output ){
	...
	if( isset($_REQUEST['id']) && $_REQUEST['id'] != '' ){
	  ...
	  if( $_REQUEST['page'] == 'blocks' ){
	    ...
	    $content = htmlspecialchars( $common->readFile( $file ) );
	  }
	} else {
  ...
  function updateData(){
    ...
    if( $_REQUEST['page'] == 'blocks' && $_REQUEST['action'] == 'save' ){
	  ...
	  $fields = array(
	    ...
	    'content'=> $_REQUEST['section_content'],
	  );
  ...
  function sectionScript( $write=true ){
	...
	$script = $common->readHTML(  'admin/assets/php/external.php' );
	...
	if( $write ){ 
	  ...
	  $common->writeFile( $original_file, $script );
	  // delete old file if necessary
  ...
  function writeConfig(){
    ...
    $escape_options = array( 'Sections', 'Templates', 'Blocks' );
	
    foreach( $config as $k => $v ){
	  if( in_array($k, $escape_options) === false ) {
        $variable = "\$config['" . $k . "']";
        $new_line = $variable . " = '" . $_REQUEST[strtolower($k)] . "';";
